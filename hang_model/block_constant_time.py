...
class ConstantODEblockTime(ODEblock):
  def __init__(self, odefunc,  opt,  device, t=torch.tensor([0, 1])):
    super(ConstantODEblockTime, self).__init__(odefunc,  opt,  device, t)
  ...
  def forward(self, x,adj,tend=None):
    ...
    if tend == 0:
      self.t = torch.tensor([self.opt['time'],0.00]).to(self.device)
    else:
      self.t = torch.tensor([0, self.opt['time']]).to(self.device)
    ...
    #   z = state_dt[-1]
    z = state_dt[1]
    # print("len of state_dt",len(state_dt))
    return z
